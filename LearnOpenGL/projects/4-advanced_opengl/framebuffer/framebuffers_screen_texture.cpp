...
int main()
{
    ...
    // Framebuffers
	// 创建一个default framebuffer 之外的framebuffer
    GLuint framebuffer;
    glGenFramebuffers(1, &framebuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);  
    // Create a color attachment texture
	// 创建一个color texture 并且绑定到framebuffer。 使用texture的原因是texture是可读写的，一旦rendering 完成后， 可以在后续draw中使用
	// sample 来读取texture数据做进一步的处理。
    GLuint textureColorbuffer = generateAttachmentTexture(false, false);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureColorbuffer, 0);
    // Create a renderbuffer object for depth and stencil attachment (we won't be sampling these)
	// 创建renderbuffer 来存储depth 和 stencil，并绑定到framebuffer 上面。注意render buffer 是只写的。
    GLuint rbo;
	...
	// 解绑 framebuffer，使用默认framebuffer
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // //////////////////////////////////////////////////
		// 绑定framebuffer 作为color 输出。这个pass 里面会完成 floor 和 cube 的绘制。
        glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
        ...
        // //////////////////////////////////////////////////
		// 解绑自创建的framebuffer，使用默认framebuffer。
		// 这里完成的工作是：对整个屏幕画两个大三角形形成一个quad，对上一次Draw的texture framebuffer进行sample，
		// 取得的sample 值在ps 中使用kernal 进行边缘检测。最后输出color。
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
