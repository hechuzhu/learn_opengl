...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))
    {
		...
		// 开启stencil buffer，开始画 cube 集合体，只要画的fragment，对应的stencil buffer 内容更新为1
		...
		// 第二次画，开启stencil test，只画 stencil value 不等于1 的fragment，并且不允许更新stencil buffer。
		// 这里使用的shader 是一个单色，用来框定区域。要框定cube 的轮廓，需要将原始的cube 在集合上稍微变大一点。
		// 这样，原始大小的fragment 区域，由于stencil value 全部为1，所以不会画出来。但是稍微大一点的外边的区域就会被画出来。
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        glStencilMask(0x00);
